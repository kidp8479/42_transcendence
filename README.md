...
The frontend uses ESLint and Prettier. Run them from inside the frontend container:
...
Or via a one-liner without entering the shell:
...
A more automatic solution (such as pre-commit hooks) will be explored later. For now, run them manually before committing.
